...
// *****************************************************************************
//  Prototype:
//      UINT8 dm_get_segment_status(unsigned char Index)
//
//  Description:
//      Get direct map segment status.
//  
//  Parameters:
//      Index: SRAM segment index (0~12).
//
...
//      ILM status.
// *****************************************************************************
UINT32 dm_get_segment_status(UINT8 u8SRAMIndex)
{
    UINT32 st;
	...
	if(u8SRAMIndex>4)
	{
        ...
        st = ECReg(R_SCRA5L+0x3*u8SRAMIndex)
           |(((UINT32)ECReg(R_SCRA5M+0x3*u8SRAMIndex)) << 8)
           |(((UINT32)ECReg(R_SCRA5H+0x3*u8SRAMIndex)) << 16);
		 
	}
	else
	{
        st = ECReg(R_SCRA0L+0x3*u8SRAMIndex)
           |(((UINT32)ECReg(R_SCRA0M+0x3*u8SRAMIndex)) << 8)
           |(((UINT32)ECReg(R_SCRA0H+0x3*u8SRAMIndex)) << 16);
	}
