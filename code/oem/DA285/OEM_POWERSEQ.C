...
void SetS0GPIO(void)
{
  ...
  EC_FAN_PWM_HI();
   ...
   USB_RE_Enable_OUTPUT; //A285D00054:
   USB_RE_Enable_HI(); //A285D00054:
  ...
  WSMbusTemp01=0x0F;
  WSMbusTemp02=0x1E;
  bRWSMBus(chSMB_TYPEC, SMbusWW, 0x54, 0x04, &WSMbusTemp01, SMBus_NoPEC);
  //A285D00053:	 	
  //
...
BYTE S5S0_STEP1(void)
{
  ...
  PM_PWRBTN_HI();
  EC_ON_HI();
  EC_ON2_HI(); //add for A285 SIT SCH change 180408  //David- for the pin change to KBID
  cPWRONFail = 0x00;
...
BYTE S5S0_CHK_095VSVDDPPG(void)
{
  ...
  //A285D00043 del for A285 SIT SCH change 180408--->Start
  //if(CHK_095VS_PGD ()) //David+ 171027 
  //{
  //  return(1);
 // }
 //A285D00043 del for A285 SIT SCH change 180408--->End
  return(0);
}
...
//
  ...
  { S5S0_SUSP_HI,         10,       0,  },  // 8
  { S5S0_USBON,            21,       0,  },  // 9 //01->21 A285D00043 modify for A285 SIT SCH change 180408
  //{ S5S0_CHK_095VSVDDPPG, 20,       1,  },  // 10 A //A285D00043 + 10->20////David del for A285 SIT SCH change 180408
...
BYTE S0S5_4S_SD_ECON_OFF(void)
{
	...
	{ 
	    LedPowerOff();//A485D00081
		EC_ON_LOW(); //
		EC_ON2_LOW();//A285D00043 add for A285 SIT SCH change 180408
		RSMRST_LOW();
	}
	return 0;
}
BYTE S0S5_4S_SD_ECON_ON(void)
{
	...
	{
		EC_ON_HI();
		EC_ON2_HI(); //A285D00043add for A285 SIT SCH change 180408
	}
...
{
  //for project design
  //PWR_GOOD_D_LOW();  ////A285D00029-   for 4S shutdown auto boot on
  TP_RESET_OFF(); //A485D00042 As EE required setting TP reset to low in S5
   BYPASS_PAD_OFF();
 //A285D00003-->End
 //for LCFC common code
 OEM_SYSTEM_S0_S5(); 
...
//
  ...
  { Sequence_Null,         0,       0,  },  // Dummy step always in the top of tabel
  { S0S5_CHK_S5S3,         1,       0,  },  // 1
  //{ S0S5_SUSP_OFF,         0,       0,  },  // 2//A485D00029-  
  { S0S5_PWRGOOD_LOW,      2,       0,  },    //A485D00029+  
  //{ S0S5_CHK_095VSVDDPPG,  0,       1,  },  // 3 //A485D00030- 
  { S0S5_APUPWR_EN_OFF,   30,       0,  },  // 4
  //{ S0S5_SYSON_OFF,       20,       0,  },  // 5//A485D00029-
  { S0S5_CHK_IMVPPOK,      1,       1,  },  // 6
  { S0S5_SUSP_OFF,         5,       0,  },  // 2//A485D00029+  
 // { S0S5_CHK_095VSVDDPPG,  0,       1,  }, ////A285D00043 del for A285 SIT SCH change 180408 // 3 //A485D00030+ 
  { S0S5_SYSON_OFF,       20,       0,  },  // 5//A485D00029+  
...
//
  ...
  //{ S0S3_SUSP_OFF,         0,       0,  },  // 2//A2485D00029-
  { S0S3_PWRGOOD_LOW,         2,       0,  }, //A285D00029+
 // { S0S3_CHK_095VSVDDPPG,  0,       1,  },  // 3 //A285D00030-
  { S0S3_APUPWR_EN_OFF,   30,       0,  },  // 4
  { S0S3_CHK_IMVPPOK,      1,       1,  },  // 5
  { S0S3_SUSP_OFF,         5,       0,  },  // 2 ////A285D00029+
  //{ S0S3_CHK_095VSVDDPPG,  0,       1,  }, ////A285D00043 del for A285 SIT SCH change 180408 // 3 //A285D00030+
...
//
    ...
    { S3S0_CHK_S3,          1,       1,  },  // 4
    { S3S0_SUSP_HI,        20,       0,  }, //10->20// A285D00043 modify for A285 SIT SCH change 180408 // 5
   // { S3S0_CHK_095VSVDDPPG,10,       1,  }, ////A285D00043 del for A285 SIT SCH change 180408 // 6
      ...
      )  // Not cut power in the battery mode for BIOS used.
    {
      if ( IS_MASK_CLEAR(ACPI_HOTKEY, b6Cmd_NoShut) && IS_MASK_CLEAR(cCmd,cC_PCHPWR_Keep))  // Not cut power in the battery mode for command used.
      {
        ...
        else
        {
          ...
          { return; }
          //if ((EC_AOU_PWR_TYPE & 0xF0) == 0)  //A485D000105-
          //{
            _nop_();
            _nop_();
            _nop_();
...
void Oem_SysPowerContrl(void)
{
  ...
  switch (SysPowState)
  {
    ...
    case SYSTEM_S5:
        ...
        if(IS_MASK_SET(EventMask1, EventFirstBootUp))
        {
            if(Read_AC_IN())
            {
                FirstBootACOffCount = 0;
                if(++FristBootACOnCount >= 5)
                {
                    FristBootACOnCount = 0;
                    CLEAR_MASK(EventMask1, EventFirstBootUp);
                }
            }
            else
            {
                FristBootACOnCount = 0;
                if(++FirstBootACOffCount >= 5)
                {
                    if(!WOLDCWakeEnGet) //A285D00072+
                    {
                    EC_ON_LOW();
                    }
                    FirstBootACOffCount = 0;
     ...
     //A285D00029: Pull high RSMRST when AC-In for Type-C Display issue --->Start 	
	if(Read_09VALWS_PWRGD() && Read_AC_IN())
	 {
	    
	    PM_PWRBTN_HI();  //A485D00066: Fix auto boot when pull high RSMRST 		
		EC_WAKE_OUT_HI();//A485D00066: Fix auto boot when pull high RSMRST 
	   
	   	if(RSMRSTDelayCnt < 22)
	   	{ RSMRSTDelayCnt++;}
        else{ RSMRST_HI();}
		...
		{RSMRSTDelayCnt=0; }
   //A285D00029+//A285D00017: Pull high RSMRST when AC-In for Type-C Display issue --->End
      if(Read_SLPS3() && Read_SLPS5())
      {
        DelayDoPowerSeq = 0;
        PWSeqStep = 1;
...
{
	if (!WOLDCWakeEnGet)  //A485D00088: keep 3v always if WOL is enabled
	{
		EC_ON_LOW();
	}
	if((!USB_PWRGET) ||
	    (IS_MASK_CLEAR(SYS_STATUS,AC_ADP) && ( SecRelativeStateOfChgL < 10) && ( RelativeStateOfChgL< 10 ))
	    || (IS_MASK_CLEAR(SYS_STATUS,AC_ADP) && (USBALWONGet)))//A485D000110//A485D00098
	{
	//EC_ON_5V_Low();
	EC_ON2_LOW();  //A285D00043+
	} //A485D00098
	AC_PRESENT_LOW();
	EC_WAKE_OUT_LOW();
