...
//---------------------------------------------------------------------------	
	...
	{ DisAOU_DC_Dis;}	
       ...
       //A285D00041:Fix sysem cannot be WoLAN from S5 (G3 then plugin AC) with magic packet
        ...
        if(0xFF == eEEPROMMarkData)
       {
            eEEPROMMarkData = 0x00;
       }
	if(IS_MASK_SET(eEEPROMMarkData, EEPROM_WOLACEn))
	{ SET_MASK(EventMask0, EnvntWOLAC); }
	else
	{ CLEAR_MASK(EventMask0, EnvntWOLAC);}
	if(IS_MASK_SET(eEEPROMMarkData, EEPROM_WOLDCEn))
	{ SET_MASK(EventMask1, EnvntWOLDC); }
	else
	{ CLEAR_MASK(EventMask1, EnvntWOLDC);}
	...
	SysPowState=SYSTEM_S5;
        ...
        SET_MASK(EventMask1, EventFirstBootUp);
        FirstBootupStatus = 0;
        #endif
	pWorkAround = 0x01;

	thermalTBlMode = PFM_Mode; //A485D00039.
	ThermalTBlMode_M= thermalTBlMode;  //A485D00091
...
void Init_ClearRam(void)
{
  ...
  temp2 = WinFlashMark;
  temp3 = WinFlashMark2;
  temp4 = afterFlashMark; // A285D00006
  ...
  WinFlashMark = temp2;
  WinFlashMark2 = temp3;
  afterFlashMark = temp4;  //A285D00006
}
...
void OEM_PWRSWWDTEn()
{
	...
	PWRBTN_WDTFLAG = TRUE;  //David+ //A285D00002 Fix auto boot on after 8S Reset
}
