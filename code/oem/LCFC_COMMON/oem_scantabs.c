...
{  // 15" US,UK and JP
  ...
  0xF4,0x1D,0x24,0x2D,0x3C,0x43,0x44,0x4D,0x6A,0x04,0x00,0x00,0x00,0xF1,0x00,0x00, // 02
...
{//A485D00035:Add Fn + Q send Q event--->Start
  //RamDebug(0xF1);
	if (temp_scanner_state.field.tFN) {
		if ( IS_MASK_SET(SYS_MISC1, ACPI_OS)&&(event == MAKE_EVENT))
		{
		  ECQEvent(PRW_OFF_SW_Pressed);    // Qevent(0x28).
		  //A285D00008 Add fan table
           if(thermalTBlMode == PFM_Mode)
			thermalTBlMode = COOL_Mode;
		else
			thermalTBlMode = PFM_Mode;
		 //A285D00008 Add fan table	
		}
	  }else{
		simple_code(0x15 ,event);
	  }
//A485D00035:Add Fn + Q send Q event--->End
...
// Function key F1 ~ F12
  ...
  18  , DO_COSTOMER_FUNCTION,     // Function key Q    [E2h]
};
                  // Reserved 0xE2 ~ 0xFF
...
void Hook_keyboard_Oem(uKEY *key,  BYTE event)
{
  if( FnCtrlSWAPGet )
  {
    if((key->field.input== 0x00)&&(key->field.output == 0x0F))// LCtrl //A285D00005
    {
      // Replaced by Fn key
      key->field.input = 0x04;
      key->field.output = 0x0C; // Fn //A285D00014: Enable Keyboard Fn& Ctrl swap function
    } else if((key->field.input == 0x04)&&(key->field.output == 0x0C)) { // Fn
      // Replaced by LCtrl key
      key->field.input = 0x00;   //A285D00005
      key->field.output = 0x0F; // LCtrl  //A285D00005
    }
  }
